...
`token.json` contains the temporary token and refresh code - see below.
...
week/year specified it will get the activities for the previous week (la week).
If the activities file already exists, use `-f` to force the get(ot herwise it
assumes the data is already there). `strava_get` checks the validity of the
temporary token in `token.json` and requests a new temporary token if it has
expired.
...
The `training_distance` is based on my quirky way of converting everything into
running kilometres. I assume I need to cycle 4km to get the equivalent of 1
running km. Swimming 1km is equivalent to running 3km. Everything else (hikes,
SUPs, kayaking, ...) is worth 0. I use this to calculate a "running equivalent"
training distance. It is probably pointless, certainly unscientific. I did say I
wrote this for me!
...
A montage of all my 2020 activities, generated by `strava_gridplot`:
...
`strava_summary -c activities/2020*` gives all the activities in the ISO year
